...
<?php
if ((isset($itemActions) && count($itemActions)) || (isset($showActions) && count($showActions))) { ?>
    use Lle\CruditBundle\Dto\Action\ItemAction;
<?php
} ?>
<?php
if (isset($listActions) && count($listActions)) { ?>
    use Lle\CruditBundle\Dto\Action\ListAction;
<?php
} ?>
use Lle\CruditBundle\Dto\Field\Field;
<?php
if ((isset($listActions) && count($listActions)) || (isset($itemActions) && count(
            $itemActions
        )) || (isset($showActions) && count($showActions))) { ?>
...
foreach ($tabs as $tab) { ?>
    ...
    if ($tab["type"] === "sublist") { ?>
        <?php
        if (!in_array($tab['linkedEntity'], $allLinkedEntity)) { ?>
            private <?= $tab["linkedEntity"] ?>CrudConfig $<?= strtolower($tab["linkedEntity"]) ?>CrudConfig,
            <?php
            $allLinkedEntity[] = $tab['linkedEntity'] ?>
        <?php
        } ?>
    <?php
    } ?>
...
<?php
if ((isset($listActions) && count($listActions)) || (isset($disabledActions['list']) && count(
            $disabledActions['list']
        ))) { ?>
    ...
    <?php
    if (isset($disabledActions['list'])) { ?>
        <?php
        foreach ($disabledActions['list'] as $key => $action) { ?>
            <?php
            if ($action === 'new') {
                $action = 'add';
            } ?>
            <?php
            if ($key === array_key_first($disabledActions['list'])) { ?>
    ...
    foreach ($listActions as $action) { ?>
        ...
        '<?= $action['label'] ?>',
        <?php
        if ($action['path']) { ?>Path::new('<?= $action['path'] ?>'),<?php
        } else { ?>null,<?php
        } ?>
        <?php
        if ($action['icon']) { ?>Icon::new('<?= $action['icon'] ?>'),<?php
        } else { ?>null,<?php
        } ?>
        )<?php
        if ($action['cssClass']) { ?>->setCssClass('<?= $action['cssClass'] ?>')<?php
        } ?><?php
        if ($action['target']) { ?>->setTarget('<?= $action['target'] ?>')<?php
        } ?>;
    <?php
    } ?>
...
<?php
if ((isset($itemActions) && count($itemActions)) || (isset($disabledActions['item']) && count(
            $disabledActions['item']
        ))) { ?>
    ...
    <?php
    if (isset($disabledActions['item'])) { ?>
        <?php
        foreach ($disabledActions['item'] as $key => $action) { ?>
            <?php
            if ($key === array_key_first($disabledActions['item'])) { ?>
...
<?php
if ((isset($showActions) && count($showActions)) || (isset($disabledActions['show']) && count(
    ...
    <?php
    if (isset($disabledActions['show'])) { ?>
        <?php
        foreach ($disabledActions['show'] as $key => $action) { ?>
            <?php
            if ($key === array_key_first($disabledActions['show'])) { ?>
