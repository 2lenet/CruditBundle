...
foreach ($items as $item) { ?>
    ...
    if (isset($item["type"]) && $item["type"] === "separator") { ?>
        ...
        HorizontalSeparatorElement::new()<?php
        if (isset($item["role"])) { ?>->setRole("<?= $item["role"] ?>")<?php
        } ?><?php
        if (isset($item["parent"])) { ?>->setParent("<?= str_replace(".", "-", $item["parent"]) ?>")<?php
        } ?>,

    <?php
    } elseif (isset($item["url"])) { ?>
        ExternalLinkElement::new("<?= $item["label"] ?>", "<?= $item["url"] ?>")<?php
        if (isset($item["icon"])) { ?>->setIcon(Icon::new("<?= $item["icon"] ?>")) <?php
        } ?><?php
        if (isset($item["role"])) { ?>->setRole("<?= $item["role"] ?>")<?php
        } ?><?php
        if (isset($item["parent"])) { ?>->setParent("<?= str_replace(".", "-", $item["parent"]) ?>")<?php
    ...
    <?php
        ...
        LinkElement::new("<?= $item["label"] ?>", <?php
        if (isset($item["route"])) { ?>Path::new("<?= $item["route"] ?>")<?php
        } else { ?>null<?php
        } ?>)->setId("<?= str_replace(".", "-", $item["label"]) ?>")<?php
        if (isset($item["icon"])) { ?>->setIcon(Icon::new("<?= $item["icon"] ?>"))<?php
        } ?><?php
        if (isset($item["role"])) { ?>->setRole("<?= $item["role"] ?>")<?php
        } ?><?php
